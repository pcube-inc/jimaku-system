...
| 2026-07-07 | 未push3コミットをpush（失効トークンを除去しブラウザ認証に切替）／出退勤：トラブル報告もシフト外無効化・サーバー側二重打刻防止を追加しattendance@9デプロイ・Pages反映確認／未コミットファイル整理（MANUAL.md分冊化確定・screenshots追加・仕様書追加）／MANUAL_STAFF.mdを最新仕様に更新（掲示板・出退勤3ボタン・スクショ撮り直し） | 別PC |
